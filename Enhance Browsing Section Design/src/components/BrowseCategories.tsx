...
export default function BrowseCategories({ onSelect, activeId }: { onSelect?: (id: string) => void; activeId?: string }) {
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {categories.map((category, index) => (
            <button
              ...
              aria-pressed={activeId === category.name}
              aria-label={`Filter by ${category.name}`}
              data-testid={`category-${category.name}`}
              className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm border border-slate-700/50 p-6 transition-all duration-300 hover:border-transparent hover:shadow-2xl hover:shadow-purple-500/20 focus:outline-none focus:ring-2 focus:ring-indigo-400"
            >
